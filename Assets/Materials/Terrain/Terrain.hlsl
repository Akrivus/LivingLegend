...
float4 GetTerrainColor(
	...
	int index)
{
	float3 uvw = float3(WorldPosition.xz * (TileScale * TILING_SCALE), Terrain[index]);
...
void GetFragmentData_float(
	UnityTexture2DArray TerrainTextures,
	UnityTexture2DArray TerrainNormals,
	float3 WorldPosition,
	float4 Terrain,
	float4 Visibility,
	float3 Weights,
	...
	out float Exploration)
{
	float4 c =
		GetTerrainColor(
			TerrainTextures, WorldPosition, Terrain, Weights, Visibility, TileScale, 0) +
		GetTerrainColor(
			TerrainTextures, WorldPosition, Terrain, Weights, Visibility, TileScale, 1) +
		GetTerrainColor(
			TerrainTextures, WorldPosition, Terrain, Weights, Visibility, TileScale, 2);
    float4 n =
		GetTerrainColor(
			TerrainNormals, WorldPosition, Terrain, Weights, Visibility, TileScale, 0) +
		GetTerrainColor(
			TerrainNormals, WorldPosition, Terrain, Weights, Visibility, TileScale, 1) +
		GetTerrainColor(
			TerrainNormals, WorldPosition, Terrain, Weights, Visibility, TileScale, 2);
	
	BaseColor = ColorizeSubmergence(c.rgb, WorldPosition.y, Terrain.w);
	Normal = n.rgba;
    Smoothness = c.r + c.g + c.b;
